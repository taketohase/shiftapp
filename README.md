...
#### 2. 従業員の方
「従業員の方はこちら」からアカウントを作成してください。なお、上の「使用例」で示したように従業員を並べて表示するとき、ユーザーIDの辞書順で表示されるため、必要があれば従業員のIDは管理者の方からあらかじめ指定しておくと良いです。次に、ヘッダーメニューの「管理者を検索」の検索フォームに、オーナーのIDを入力して検索し、「登録リクエストを送る」をクリックしてください。
#### 3. オーナーの方
